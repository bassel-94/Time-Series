...
p1 = ggplot(data4[data4$Type=="Crude",])+
  ...
  xlab("Month")+
  ylab("Price in dolars (1994)")
p2 = ggplot(data4[data4$Type=="Miles",])+
  aes(x=as.factor(month),y=Value)+
  geom_boxplot()+
  stat_summary(fun=median, geom="smooth", aes(group=1))+
...
### Autocorrelation of the covariates 

Similarly, we will explore the autocorrelation of each of the covariates of interest as a function of the lag. Their corresponding plots below indicate a high autocorrelation with respect to their lag, further confirming the non stationarity found in all of the covariates.
...
Another way of checking stationary of time series is the Augmented Dickey-Fuller test.
...
```{r}
ggplot(errors)+aes(x=AR,y=ERR,colour=as.factor(MA))+geom_point()+ylab("Error")
```
...
p1=ggplot(to_plot)+aes(x=Date,y=Value,colour=Type)+
  geom_line()+
  xlab("Monthly Change")+
  ggtitle("Crude Oil Price",subtitle = "First difference")
...
p2=ggplot(to_plot)+aes(x=Date,y=Value,colour=Type)+
  geom_line()+
  xlab("Monthly Price")+
  ggtitle("Crude Oil Price",subtitle = "Real Vs. Fitted")
grid.arrange(p1,p2)
```


As can be observed from the plots above our model captures quite well the evolution of both the differentiated series and that of the real series. What our model fails to fully take into account is the variance of the series, it has a lower variance than the real series.
...
```{r}
ggplot(errors)+aes(x=AR,y=ERR,colour=as.factor(MA))+geom_point()
```

The plot above shows the empirical error of the models. Based on it's output we chose a model with AR=1 and MA=2.
...
p1=ggplot(to_plot)+aes(x=Date,y=Value,colour=Type)+
  geom_line()+
  xlab("Monthly Change")+
  ggtitle("Miles Travelled",subtitle = "First difference")
...
p2=ggplot(to_plot)+aes(x=Date,y=Value,colour=Type)+
  geom_line()+
  xlab("Monthly Miles")+
  ggtitle("Miles Travelled",subtitle = "Real Vs. Fitted")
grid.arrange(p1,p2)
```

Our model reconstructs quite well both the differentiated moving averages of the original series and the moving averages themselves. The only problem is that the predicted values are somewhat lower than the real ones. This time the model also fits quite well the variance of the series.
