---
title: "High Dimensional Time Series assignmnet"
...
* U.S. No 2 Diesel Retail Prices (referred bellow as diesel prices): It is a general value of diesel prices in the United States, the series is downloaded from EIA. The values of the series are expressed in Dollars per Gallon. We did not use this series in the analysis, we included it for the sake of completion of the data.
...
As we are working on differentiated series we will actually fit ARIMA models by using the ARMA R function of the series of the first difference of the original variables.
...
We can further understand the quality of the model by analising the distribution of the errors which we can see in figure \ref{fig:er1}.

```{r er1, fig.align='center', fig.width=5, fig.height=3.5, echo = FALSE, warning=FALSE, fig.cap="Normal QQ plot of the errors of Crude oil prices"}
qqnorm(model$residuals)
```

The plot above shows that the distribution of the errors is affected by the existence of some outliers in the residual series. These are due to the differences at the peak shown in the previous plot.
...
For such parameters, we get the following results :knitr::kable(to_table, 
  caption = 'P value of the statistic Augmented Dickey-Fuller', 
  col.names = c("Series name", "Lag order", "p-value")
)
...
We can further understand the quality of the model by analyzing the distribution of the errors.

```{r, fig.align='center', fig.width=5, fig.height=3.5, echo = FALSE, warning=FALSE, fig.cap="Normal QQ plot of the errors of Miles traveled time series"}
qqnorm(model$residuals)
```

As was the case with the previous model the plot above shows that the distribution of the errors is affected by the existence of some outliers in the residual series. These are due to the differences at the peak shown in the previous plot.
...
The output bellow shows the selected lag for various informational criteria. We chose to use 1 in our model because it is selected by one of the criterion and it is easier to interpret.
...
The plot below shows side by side the fitted VAR model against the differentiated (i.e. stationary) time series and the fitted VAR model against the original time series.
...
### Checking the errors of the VAR model

As with the ARMA/ARIMA models we used it is important to check the distribution of the errors.

```{r er2, fig.align='center', fig.width=7, fig.height=3.5, echo = FALSE, warning=FALSE, fig.cap="QQ norm plots and Shapiro test results"}
par(mfrow=c(1,3))
qqnorm(model$varresult$Gasoline$residuals,main="Gasoline")
qqnorm(model$varresult$Crude$residuals,main="Crude")
qqnorm(model$varresult$RA_Miles$residuals,main="Miles")
to_shapiro<-data.frame(Series=0,Pi_Value=0,Conclusion=0)
s_gas<-shapiro.test(model$varresult$Gasoline$residuals)
s_cru<-shapiro.test(model$varresult$Crude$residuals)
s_mil<-shapiro.test(model$varresult$RA_Miles$residuals)
to_shapiro[1,]<-c("Gasoline",round(s_gas$p.value,6),0)
to_shapiro[2,]<-c("Crude",round(s_cru$p.value,6),0)
to_shapiro[3,]<-c("Miles",round(s_mil$p.value,6),0)
to_shapiro[as.numeric(to_shapiro[,2])<0.05,3]<-"Not normally distributed"
```

```{r, echo = FALSE, fig.align='center', fig.cap="Results of the sharpio wilk normality test for the ARMA model"}
knitr::kable(to_shapiro, 
  caption = 'P value of the sharpio statistic Augmented Dickey-Fuller', 
  col.names = c("Series name", "p-value", "Conclusion")
)
```

As we can see the errors in figure \ref{fig:er2} are not normally distributed, the plots suggest that this is again due to the outliers in the data. This shows a difficulty in predicting prices of real-world commodities as geopolitical, economic and even public health shocks can upend well established common sense expectations.
...
```{r er3, fig.align='center', fig.width=7, fig.height=3.5, echo = FALSE, warning=FALSE, fig.cap="QQ norm plots and Shapiro test results"}
to_shapiro<-data.frame(Series=0,Pi_Value=0,Conclusion=0)
for(i in 1:3){
  to_shapiro[i,1]<-colnames(model.lasso$residuals)[i]
  to_shapiro[i,2]<-shapiro.test(model.lasso$residuals[,i])$p.value
  if(as.numeric(to_shapiro[i,2])<0.05){
    to_shapiro[,3]<-"Not normally distributed"
  }
  else
    to_shapiro[,3]<-"Normally distributed"
}
par(mfrow=c(1,3))
for(i in 1:3){
  qqnorm(model.lasso$residuals[,i],main=colnames(model.lasso$residuals)[i])
}
```

```{r, echo = FALSE, fig.align='center', fig.cap="Results of the sharpio wilk normality test for penalized ARMA model"}
knitr::kable(to_shapiro, 
  caption = 'P value of the sharpio wilk statistic', 
  col.names = c("Series name", "p-value", "Conclusion")
)
```

We can again observe in figure\ref{fig:er3} that the outliers in the data affect the quality of the model by creating outliers in the errors too.

## Conclusion

In our project we tried to create a model that would predict and help in understanding the evolution of gasoline prices in the United States of America in the last 26 years. In order to do that we used both univariate and multivariate models. We began by checking for stationarity and seasonality in the data. After we smoothed and differentiated the data we fitted univariate models on two of those series, namely on Crude Oil Prices and Miles travelled per month. 

The fitted univariate models were quite effective in modeling the original series. They had some problems due to the existence of some outlier values (created by the economic crises of 2008 and 2020). A similar pattern was observed in the VAR models fitted (both with and without LASSO).