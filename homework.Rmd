...
simulate_var = function(n, T) {
  ...
  for (l in seq_along(n)){
    ...
    #-- Generate a VAR(1) model of dimension l with T=100 observations
    for (j in 2:T){
      X[,j]=A%*%X[,(j-1)]+t(t(rnorm(p)))
    }
    
    #-- Estimate a VAR(1) model of dimension l from T=100 observations
    Estim=VAR(t(X),1)
    A_hat=Estim$Phi
  ...
  plot(error, type = 'b', lwd = 2, 
       xlab = "Values of n", ylab = "Error", axes = FALSE,
       main = "Estimation error of different values of n")
...
```{r}
#-- test for T=100
simulate_var(c(2,5,10,20),100)

#-- test for T=500
simulate_var(c(2,5,10,20),500)
